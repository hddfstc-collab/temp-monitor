...
function requestWithTimeout(promise, ms = 4000) {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('Tuya API 무응답 타임아웃')), ms);
    promise
...
async function collect() {
  ...
  try {
    ...
    for (const key of keys) {
      try {
        ...
        if (res && res.success) {
          ...
          console.log(`✅ [${deviceName}] 수집 및 파이어베이스 저장 완료`);
        } else {
          console.log(`⚠️ [${deviceName}] 투야 서버가 에러 반환:`, res ? res.msg : '응답없음');
        }
        
        await new Promise(resolve => setTimeout(resolve, 500));

      } catch (deviceError) {
        console.error(`🔥 [${key}] 수집 실패 사유:`, deviceError.message);
      }
    }
    ...
    console.error("🔥 [치명적 크래시] 파이어베이스 통신 자체 실패:", e.message);
    process.exit(1);
  }
}
